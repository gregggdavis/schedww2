...
{
    public class clsDevExpressFormPrinting : Link
    {
        ...
        #region Printing to TIFF Not needed now since devexpress supports saving images with different formats
        ...
        #region Replaced with CreateDetail which is called every page also
        /// <summary>
            ...
            HorizontalAlignment hAlignment)
        {
            ...
            {
                Single fontHeight = mp.Graphics().Font.GetHeight();//printFont.GetHeight(mp.Graphics());
            ...
            ref Single extendedHeight, out bool ScanForChildControls)
        {
            ...
            else  //multiline
            {
                ...
                {
                    g.Font = printFont;
                    //g.StringFormat = new StringFormat();
                    
                    SizeF gsize = g.MeasureString(tb.Text.Substring(pos),Convert.ToInt32(areaForOneLineOfText.Width),new StringFormat());
                    SizeF goneChar = g.MeasureString("C", Convert.ToInt32(areaForOneLineOfText.Width), new StringFormat());

                    float widthDifference = gsize.Width - areaForOneLineOfText.Width;
                    if (widthDifference <= 0)
                        charactersFitted = tb.Text.Substring(pos).Length;
                    else
                    {
            ...
            switch (typePrint)
            {
                case ParentControlPrinting.BeforeChilds:
                    ...
                    else
                    {
                        ...
                        mp.DrawString(c.Text, printFont, Color.Black, x + h, y, c.Width - h - h, h);
                        mp.EndPrintUnit();
            ...
            ref Single extendedHeight, out bool ScanForChildControls)
        {
            ...
            switch (typePrint)
            {
                case ParentControlPrinting.BeforeChilds:
                    ...
                    mp.DrawString(tp.Text, printFont, Color.Black, x, y + (tc.ItemSize.Height - h) / 2, tp.Width, h);
        ...
        public class MultiPageManagement
        {
            ...
            public Single _realPageTop, _realPageHeight, _UsablePageHeight;
            public Single _realPageLeft, _realPageRight;
            public Single _CurrentPageTop, _CurrentPageBottom;
            public int _PageNumber = 0;
            //private PrintPageEventArgs _Ev;
            public BrickGraphics _G;
            public bool _PrintUnit;
            public bool _PrintInCurrentPage;
            public Single _PrintUnitPullDown;
            public bool _pageNumbering;
            public Font _FontForPageNumering;
            public string _PageNumberingFormat;
            public bool isHeader = false;
            public BrickGraphics Graphics()
            {
                return _G;
            }
            ...
            public Single _ConvertToPage(Single y)
            {
            ...
            public void DrawLines(Pen pen, Single x1, Single y1, Single x2, Single y2)
            {
                if (PrintUnitIsInCurrentPage())
                {
                    ...
                    //g.DrawLine()
                    _G.DrawLine(points[0],points[1],pen.Color, pen.Width);
                    //_G.DrawLines(pen, points);
                }
            }
            // change brush to color
            public void DrawString(string s, Font printFont, Color brush, Single x, Single y, Single w, Single h)
            {
            ...
            public void DrawRectangle(Pen pen, Single x, Single y, Single w, Single h)
            {
                if (PrintUnitIsInCurrentPage())
                {
                    ...
                    _G.DrawRect(new RectangleF(x, yPage, w, h), BorderSide.All, _G.BackColor, pen.Color);
            ...
            public void DrawImage(Image image, Single x, Single y, Single w, Single h)
            {
                ...
                {
                    Single yPage = _ConvertToPage(y);
                    _G.DrawImage(image, new RectangleF(x, yPage, w, h)); //x,yPage, w, h);
                }
            }
